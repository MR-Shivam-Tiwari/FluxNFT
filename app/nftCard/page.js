    ...
    <div className="group glass rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-500 cursor-pointer select-none">
      ...
      <div className="relative overflow-hidden h-64 rounded-t-xl bg-gradient-to-br from-[#7F5AF0]/30 to-[#00E0FF]/30">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 flex items-end p-5 transition-opacity duration-300">
          <Link href={`/exploreNFTs/buynft/${nft.id}`} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#7F5AF0] to-[#00E0FF] rounded-lg font-semibold hover:shadow-lg transition-all">
        ...
        >
          <Heart
            size={24}
            className={`transition-colors duration-300 ${nft.liked ? "fill-red-500 text-red-500" : "text-gray-300"}`}
          />
        </button>

        {/* Chain badge */}
        <div
          className={`absolute top-4 left-4 px-4 py-1 rounded-full text-xs font-semibold backdrop-blur-md border border-[#00e0ff55] ${chainColors[nft.chain] || "bg-gray-600/30 text-gray-400"
            }`}
        >
          {nft.chain}
        </div>
      ...
      {/* Content */}
      <div className="p-5">
        <h3 className="font-bold text-lg mb-1 truncate group-hover:text-[#00E0FF] transition-colors duration-300">{nft.name}</h3>
        <p className="text-sm text-gray-400 mb-3 truncate">by {nft.creator || "Anonymous"}</p>

        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-400">Price</p>
            <p className="text-lg font-extrabold gradient-text">{nft.price} ETH</p>
          </div>
          <button
          ...
          >
            <ExternalLink size={20} className="text-gray-300 hover:text-[#00E0FF] transition-colors duration-300" />
          </button>
        </div>
      </div>
    </div>
